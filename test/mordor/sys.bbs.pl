#============================================================================================================
#
#	System gerenciamento - keijiban module
#	sys.bbs.pl
#	---------------------------------------------------------------------------
#	2004.01.31 start
#
...
sub SetMenuList
{
	...
	$Base->SetMenu('Lista de keijiban', "'sys.bbs','DISP','LIST'");
	
	# システム管理権限のみ
	if ($pSys->{'SECINFO'}->IsAuthority($pSys->{'USER'}, $ZP::AUTH_SYSADMIN, '*')) {
		$Base->SetMenu('Criação de keijiban', "'sys.bbs','DISP','CREATE'");
		$Base->SetMenu('<hr>', '');
		$Base->SetMenu('Keijiban category lista', "'sys.bbs','DISP','CATEGORY'");
	}
}
...
sub PrintBBSList
{
	...
	$Page->Print("</select><input type=button value=\"　Exibição　\" onclick=");
	...
	$Page->Print("<input type=button value=\"Mudança de category\" $common2,'CATCHANGE')\"> ")	if (1);
	$Page->Print("<input type=button value=\"Informação atualização\" $common1,'UPDATE')\"> ")		if ($isSysad);
	$Page->Print("<input type=button value=\"index atualização\" $common1,'UPDATEBBS')\"> ")	if (1);
	$Page->Print("<input type=button value=\"　Apagação　\" $common2,'DELETE')\" class=\"delete\"> ")		if ($isSysad);
...
sub PrintBBSCreate
{
	...
	$Page->Print("<tr><td class=\"DetailTitle\">Keijiban directory</td><td>");
	$Page->Print("<input type=text size=60 name=BBS_DIR value=\"[ディレクトリ名]\"></td></tr>\n");
	$Page->Print("<tr><td class=\"DetailTitle\">Keijiban nome</td><td>");
	$Page->Print("<input type=text size=60 name=BBS_NAME value=\"[Nome do keijiban]＠2-ch keijiban\"></td></tr>\n");
	$Page->Print("<tr><td class=\"DetailTitle\">Descrição</td><td>");
	$Page->Print("<input type=text size=60 name=BBS_EXPLANATION value=\"[Descrição]\"></td></tr>\n");
	...
	$Page->Print("</select></td></tr>\n");
	$Page->Print("<tr><td colspan=2><hr></td></tr>");
	$Page->Print("<tr><td colspan=2 align=left><input type=button value=\"　Criação　\" ");
