...
sub PrintLimitSetting
{
	...
	my $setWriteMax		= $Setting->Get('timecount')||0;
	my $setContinueMax	= $Setting->Get('timeclose')||0;
	my $setNoName		= $Setting->Get('NANASHI_CHECK');
	my $setProxy		= $Setting->Get('BBS_PROXY_CHECK');
	my $setOverSea		= $Setting->Get('BBS_BR_CHECK');
	...
	$Page->Print("<tr><td class=\"DetailTitle\">掲示板読取専用</td><td><select name=BBS_READONLY>");
	$Page->Print("<option value=on $selROon>読取専用");
	$Page->Print("<option value=caps $selROcaps>キャップのみ可");
	$Page->Print("<option value=none $selROnone>書き込み可");
	...
	$Page->Print("<input type=checkbox name=BBS_BR_CHECK $setOverSea value=on>有効</td></tr>");
	...
	$Page->Print("直近<input type=text size=5 name=timeclose value=\"$setContinueMax\" style=\"text-align: right\">書き込みのうち、");
	$Page->Print("一人が<input type=text size=5 name=timecount value=\"$setWriteMax\" style=\"text-align: right\">回まで書き込み可");
	$Page->Print("</td></tr>");
